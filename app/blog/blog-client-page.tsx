...
export default function BlogClientPage() {
  const row1Posts = BLOG_POSTS.slice(0, 3)
  const row2Posts = BLOG_POSTS.slice(3, 6)
  const featurePost = BLOG_POSTS[6]
    ...
    <main className="min-h-screen bg-black text-white">
      ...
      <section className="bg-black text-white pt-32 pb-16 md:pt-48 md:pb-24 border-b border-white/10 relative overflow-hidden">
        ...
        <div className="mx-auto max-w-[1600px] px-6 md:px-10 relative z-10">
          <motion.div {...fadeUp()}>
            <p className="font-mono text-xs uppercase tracking-[0.3em] text-[#E515AB]">/ Blog</p>
              ...
              >
                ...
                <div className="flex flex-col flex-grow">
                  <div className="flex items-center gap-3 mb-4">
                    <span className="font-mono text-[10px] uppercase tracking-[0.2em] text-[#E515AB] font-bold">{post.category}</span>
                  ...
                  <Link href={`/blog/${post.slug}`}>
                    <h3 className="font-display text-2xl md:text-[1.75rem] font-bold leading-[1.1] tracking-[-0.02em] text-black mb-4 group-hover:text-[#E515AB] transition-colors line-clamp-3">
                      {post.title}
                    </h3>
                  </Link>
                  ...
                  <Link
                    href={`/blog/${post.slug}`}
                    className="inline-flex items-center gap-3 font-mono text-[11px] uppercase tracking-[0.15em] font-bold text-black group-hover:text-[#E515AB] transition-colors mt-auto w-fit"
                  >
                    Read Article
                    <span className="block h-px w-6 bg-gradient-to-r from-[#E515AB] to-[#FFBD1E] transition-all duration-300 group-hover:w-10" />
                  </Link>
                </div>
              </motion.article>
            ))}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10 lg:gap-12">
            {row2Posts.map((post, i) => (
              ...
              >
                ...
                <div className="flex flex-col flex-grow">
                  <div className="flex items-center gap-3 mb-4">
                    <span className="font-mono text-[10px] uppercase tracking-[0.2em] text-[#FFBD1E] font-bold">{post.category}</span>
                    <span className="h-px w-4 bg-white/20" />
                    <span className="font-mono text-[10px] uppercase tracking-[0.2em] text-white/50">{post.readTime}</span>
                  </div>
                  <Link href={`/blog/${post.slug}`}>
                    <h3 className="font-display text-2xl md:text-[1.75rem] font-bold leading-[1.1] tracking-[-0.02em] text-white mb-4 group-hover:text-[#FFBD1E] transition-colors line-clamp-3">
                      {post.title}
                    </h3>
                  </Link>
                  ...
                  <Link
                    href={`/blog/${post.slug}`}
                    className="inline-flex items-center gap-3 font-mono text-[11px] uppercase tracking-[0.15em] font-bold text-white group-hover:text-[#FFBD1E] transition-colors mt-auto w-fit"
                  >
                    Read Article
                    <span className="block h-px w-6 bg-gradient-to-r from-[#E515AB] to-[#FFBD1E] transition-all duration-300 group-hover:w-10" />
                  </Link>
      ...
      <section className="bg-gradient-to-br from-[#E515AB] via-[#FA1702] to-[#FFBD1E] text-white py-24 md:py-36 relative overflow-hidden">
        <div className="mx-auto max-w-[1600px] px-6 md:px-10">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2">
              ...
              <div className="flex flex-col justify-center p-10 md:p-16 lg:p-20 relative z-10 lg:-ml-24">
                <div className="flex items-center gap-4 mb-6">
                  <span className="inline-flex rounded-full bg-white/10 backdrop-blur-sm px-4 py-1.5 font-mono text-[10px] uppercase tracking-[0.2em] text-[#FFBD1E] font-bold border border-white/20">
                    {featurePost.category}
                  </span>
                  <span className="font-mono text-[10px] uppercase tracking-[0.2em] text-white/50">{featurePost.readTime}</span>
                </div>

                <h2 className="font-display text-[clamp(2rem,3.5vw,3.5rem)] font-bold tracking-[-0.03em] leading-[1.05] text-white mb-6 group-hover:text-transparent group-hover:bg-clip-text group-hover:bg-gradient-to-r group-hover:from-[#E515AB] group-hover:to-[#FFBD1E] transition-all duration-300">
                  {featurePost.title}
                </h2>
